...
DEFAULT = ROOT / "outputs" / "repaired_model_evaluation_v2"
MODELS = ("ALIGNN-FF", "CHGNet", "M3GNet", "MACE-MP")
VIEWS = ("mp_native", "alexmp20_native", "alex_pbe_native", "common_pool")
...
STYLES = {"mp_native": (0, (5, 2)), "alex_pbe_native": (0, (1.2, 1.8)), "common_pool": "solid", "audit_view": (0, (1.2, 1.8))}
VIEW_LABELS = {"mp_native": "MP-native", "alexmp20_native": "alex-mp-20", "alex_pbe_native": "Alexandria-PBE", "common_pool": "Common pool", "audit_view": "Audit policy"}
...
def fig1(curves: pd.DataFrame, source: Path, out: Path) -> None:
    ...
    for ax, value, ylabel, letter in zip(axes, ("stable_yield", "recall"), ("Stable yield@K", "Recall of view-defined stable set"), ("a", "b")):
        ...
        ax.text(1000, 0.03, "1,000", transform=ax.get_xaxis_transform(), rotation=90, ha="right", va="bottom", fontsize=6.3, color="#65707D")
        ax.set_xlim(1, 10000)
...
def dominant_tables(metrics: pd.DataFrame, topk: pd.DataFrame, bands: pd.DataFrame, ratio_ci: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    scope = "hull_construction_sensitivity"
    label_bands = bands[bands.scope.eq(scope)][["model_name", "metric", "spread"]].rename(columns={"spread": "endpoint_band"})
    spread_scope = f"{scope}_between_model_spread"
    model_spread = bands[bands.scope.eq(spread_scope)].groupby("metric", as_index=False).spread.median().rename(columns={"spread": "median_between_model_spread"})
    dominance = label_bands.merge(model_spread, on="metric").merge(
        ratio_ci[ratio_ci.scope.eq(scope)][["model_name", "metric", "ratio_ci_low_95", "ratio_ci_high_95"]],
        on=["model_name", "metric"], how="left",
    )
    dominance = dominance[dominance.metric.isin(METRICS)].copy()
    dominance["endpoint_sensitivity_to_model_spread_ratio"] = dominance.endpoint_band / dominance.median_between_model_spread
...
def fig4(metrics: pd.DataFrame, topk: pd.DataFrame, bands: pd.DataFrame, ratio_ci: pd.DataFrame, deltas: pd.DataFrame, winners: pd.DataFrame, source: Path, out: Path) -> None:
    dominance, ranks, point = dominant_tables(metrics, topk, bands, ratio_ci)
    ...
    for offset, model in zip(offsets, MODELS):
        ...
        lower = np.maximum(0.0, d.endpoint_sensitivity_to_model_spread_ratio - d.ratio_ci_low_95)
        upper = np.maximum(0.0, d.ratio_ci_high_95 - d.endpoint_sensitivity_to_model_spread_ratio)
        axa.errorbar(x, d.endpoint_sensitivity_to_model_spread_ratio, yerr=np.vstack([lower, upper]), fmt=MARKERS[model], color=COLORS[model], ms=4.4, capsize=2, elinewidth=0.75, markeredgecolor="white", markeredgewidth=0.45, label=model, zorder=3)
    ...
    axa.set_xticks(range(len(METRICS)), [METRIC_LABELS[m] for m in METRICS]); axa.set_ylabel("Endpoint band / median four-model spread")
    axa.set_title("Endpoint sensitivity relative to model spread", pad=4)
    ...
    axb.set_xticks(x, ["MP", "alex-mp-20", "Alex-PBE", "Common pool"], rotation=18, ha="right")
    ...
    for metric in METRICS:
        ...
        axc.scatter(d.median_between_model_spread, d.endpoint_band, s=28, color=[COLORS[m] for m in d.model_name], edgecolor="white", linewidth=0.45, zorder=3)
        axc.annotate(METRIC_LABELS[metric], (d.median_between_model_spread.median(), d.endpoint_band.median()), xytext=(4, 4), textcoords="offset points", fontsize=6.0, color="#323840")
    low, high = 0.04, 0.40
    axc.plot([low, high], [low, high], color="#8F4B45", linestyle=(0, (4, 3)), linewidth=0.85)
    axc.set_xlim(low, high); axc.set_ylim(low, high); axc.set_xlabel("Median four-model spread"); axc.set_ylabel("Endpoint band")
...
def main() -> None:
    ...
    ratio_ci = pd.read_csv(a.input / "endpoint_sensitivity_to_model_spread_ratio_bootstrap.csv")
    ...
    fig4(metrics, topk, bands, ratio_ci, deltas, winners, source, a.out)
