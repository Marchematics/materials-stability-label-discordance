...
def figure1(curves: pd.DataFrame, source_dir: Path, figure_dir: Path) -> dict[str, object]:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(17.1 * CM, 8.1 * CM), gridspec_kw={"wspace": 0.18})
    panels = [("stable_yield", "Stable yield", "Stable candidates / candidates evaluated"), ("recall", "Recall", "Fraction of label-defined stable set recovered")]
    for ax, (column, title, ylabel) in zip(axes, panels):
        for model in REAL_MODELS:
            for view in DISCOVERY_LABEL_VIEWS:
                data = sample[(sample["model_name"].eq(model)) & (sample["label_view"].eq(view))]
                ax.plot(
                    data["rank"], data[column],
                    color=MODEL_COLORS[model], linestyle=VIEW_STYLES[view],
                    linewidth=1.7 if view == "audit_view" else 1.25,
                    alpha=0.96 if view == "audit_view" else 0.82,
                )
        ax.axvline(1000, color="#6E7781", linestyle=(0, (4, 3)), linewidth=0.75, zorder=0)
        ax.text(1150, 0.975, "1,000-candidate budget", transform=ax.get_xaxis_transform(), color="#59616B", fontsize=6.1, va="top")
        ax.set_xlim(0, 36_801)
        ax.set_ylim(0, 1.0)
        ax.set_xticks([0, 5000, 10000, 20000, 30000, 36801])
        ax.xaxis.set_major_formatter(FuncFormatter(lambda x, _: f"{int(x):,}"))
        ax.set_xlabel("Number of candidates evaluated, K")
        ax.set_ylabel(ylabel)
        ax.set_title(title, pad=5)
        clean_axis(ax)
    panel_label(axes[0], "a")
    panel_label(axes[1], "b")
    model_handles = [Line2D([0], [0], color=MODEL_COLORS[m], marker=MODEL_MARKERS[m], markersize=3.2, linewidth=1.8, label=m) for m in REAL_MODELS]
    view_handles = [Line2D([0], [0], color="#303030", linestyle=VIEW_STYLES[v], linewidth=1.6, label=VIEW_LABELS[v]) for v in DISCOVERY_LABEL_VIEWS]
    fig.subplots_adjust(bottom=0.27, top=0.90)
    first = fig.legend(model_handles, [h.get_label() for h in model_handles], loc="lower center", bbox_to_anchor=(0.33, 0.02), ncol=4, columnspacing=1.0, handlelength=2.2)
    fig.add_artist(first)
    fig.legend(view_handles, [h.get_label() for h in view_handles], loc="lower center", bbox_to_anchor=(0.79, 0.02), ncol=3, columnspacing=1.0, handlelength=2.8)
...
def figure2(rolling: pd.DataFrame, density: pd.DataFrame, source_dir: Path, figure_dir: Path, metadata: dict[str, object]) -> dict[str, object]:
    ...
    fig = plt.figure(figsize=(17.1 * CM, 10.8 * CM))
    gs = fig.add_gridspec(2, 1, height_ratios=[0.58, 2.25], hspace=0.08)
    ...
    ax.axvspan(0.0, 0.04, color="#F6B6B2", alpha=0.36, linewidth=0)
    ...
    for pair, data in rolling.groupby("source_pair", sort=False):
        ...
        ax.plot(x, y, color=color, linewidth=1.75, label=pair)
    ax.text(0.005, 0.285, "Near-threshold\nlabel-risk zone", color="#8C3D3A", fontsize=6.7, va="top")
    ax.text(0.196, 0.286, "40 meV window; Wilson 95% CI\nwindows with n < 1,000 masked", color="#4D5359", fontsize=6.4, va="top", ha="right")
    ax.set_xlim(0, 0.20)
    ax.set_ylim(0, 0.30)
    ...
    ax.legend(loc="upper right", bbox_to_anchor=(0.99, 0.82), handlelength=2.6)
    clean_axis(ax)
...
def figure3(denom: pd.DataFrame, conflicts: pd.DataFrame, decomp: pd.DataFrame, source_dir: Path, figure_dir: Path) -> dict[str, object]:
    ...
    fig, axes = plt.subplots(1, 3, figsize=(17.1 * CM, 8.5 * CM), gridspec_kw={"width_ratios": [1.0, 1.0, 1.24], "wspace": 0.70})
    ...
    for yi, value in zip(y, d["n_rows"]):
        ax.text(value + 500, yi, f"{value:,}", va="center", fontsize=5.9)
    ax.set_xlabel("Rows")
    clean_axis(ax, "x")
    panel_label(ax, "a")
    ...
    for yi, rate, count in zip(y, c["conflict_rate"], c["conflict_n"]):
        ax.text(100 * rate - 0.25, yi, f"{100*rate:.1f}% ({count:,})", va="center", ha="right", fontsize=5.5, color="white")
    ...
    bar_names = ["Native", "Reconstructable", "Common pool"]
    pieces = {
    ...
    for name, values in pieces.items():
        ...
        for yi, lv, value in zip(y, left, values):
            if value >= 100:
                ax.text(lv + value / 2, yi, f"{int(value):,}", ha="center", va="center", fontsize=5.8, color="#222222")
        left += values
    ax.set_yticks(y, bar_names)
    ax.set_xlim(0, 6100)
    ax.xaxis.set_major_formatter(FuncFormatter(lambda x, _: f"{int(x/1000)}k" if x else "0"))
    ax.set_xlabel("Rows")
    ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.36), ncol=2, columnspacing=0.8, handlelength=1.6)
    clean_axis(ax, "x")
    panel_label(ax, "c")
    fig.subplots_adjust(bottom=0.25, top=0.93, left=0.08, right=0.99)
...
def figure4(dominance: pd.DataFrame, slope: pd.DataFrame, source_dir: Path, figure_dir: Path) -> dict[str, object]:
    ...
    fig, axes = plt.subplots(1, 3, figsize=(17.1 * CM, 8.8 * CM), gridspec_kw={"width_ratios": [1.22, 1.18, 1.0], "wspace": 0.48})

    ax = axes[0]
    metrics = list(PRIMARY_METRICS)
    xmap = {m: i for i, m in enumerate(metrics)}
    jitter = {m: v for m, v in zip(REAL_MODELS, [-0.18, -0.06, 0.06, 0.18])}
    for model in REAL_MODELS:
        data = dominance[dominance["model_name"].eq(model)]
        ax.scatter(
            [xmap[m] + jitter[model] for m in data["metric"]],
    ...
    ax.axhline(1, color="#8B2D2D", linestyle=(0, (4, 3)), linewidth=0.8)
    ax.set_yscale("log")
    ax.set_ylim(0.55, 15)
    ...
    ax.set_ylabel("Label-view band / between-model margin")
    ax.text(4.35, 1.08, "equal influence", ha="right", va="bottom", color="#8B2D2D", fontsize=6.0)
    clean_axis(ax)
    panel_label(ax, "a")
    ...
    ax.set_xlim(-0.18, len(views) - 0.82)
    ax.set_ylim(4.35, 0.65)
    ax.set_yticks([1, 2, 3, 4])
    ax.set_xticks(x, view_labels, rotation=30, ha="right")
    ax.set_ylabel("F1 rank among real models")
    ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.42), ncol=2, columnspacing=0.9, handlelength=1.8)
    clean_axis(ax)
    panel_label(ax, "b")
    ...
    ax.set_xlabel("Median between-model\nmargin")
    ax.set_ylabel("Within-model\nlabel-view band")
    metric_handles = [Line2D([0], [0], marker="o", color="none", markerfacecolor=METRIC_COLORS[m], markeredgecolor="white", markersize=5, label=METRIC_LABELS[m]) for m in PRIMARY_METRICS]
    ax.legend(handles=metric_handles, loc="lower center", bbox_to_anchor=(0.5, -0.58), ncol=1, handletextpad=0.3, labelspacing=0.25)
    clean_axis(ax, "both")
    panel_label(ax, "c")
    fig.subplots_adjust(bottom=0.35, top=0.93, left=0.07, right=0.99)
...
def figure5(exact: pd.DataFrame, unsupported: pd.DataFrame, source_dir: Path, figure_dir: Path) -> dict[str, object]:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(17.1 * CM, 7.8 * CM), gridspec_kw={"width_ratios": [1.4, 1.0], "wspace": 0.34})
    ...
    for i, (column, label, color) in enumerate(
        [
            ("mp_native_stable_yield", "MP-native", "#8FBBD9"),
            ("consensus_stable_yield", "Consensus", "#7BBF9A"),
            ("audit_view_stable_yield", "Audit view", "#E59A58"),
        ]
    ):
        ax.bar(x + (i - 1) * width, screen[column], width, color=color, label=label)
    ax.scatter(x, screen["source_uncertain_fraction"], color="#8B2D2D", marker="D", s=18, label="Source-uncertain", zorder=3)
    ax.set_xticks(x, names, rotation=25, ha="right")
    ax.set_ylim(0, 0.62)
    ax.set_ylabel("Fraction of exact-matched candidates")
    ...
    for column, label, color in [
        ...
        ax.bar(x, values, bottom=bottom, color=color, width=0.62, label=label)
        bottom += values
    ax.set_xticks(x, names)
    ax.set_ylim(0, 1.02)
    ax.set_ylabel("Candidate-pool fraction")
    ax.text(0.5, 0.50, "No exact SourceAware\nlabels assigned", transform=ax.transAxes, ha="center", va="center", fontsize=7.0, color="#4D5359")
    ax.legend(loc="upper center", ncol=2, columnspacing=0.8)
    clean_axis(ax)
    panel_label(ax, "b")
    fig.subplots_adjust(bottom=0.23, top=0.93)
