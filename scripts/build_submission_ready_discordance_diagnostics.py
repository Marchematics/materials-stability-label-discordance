...
def build_denominator_audit(alex: pd.DataFrame, matches_all: pd.DataFrame, strict: pd.DataFrame, scores: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = [
        {
            "step": "MatterGen alex-mp-20 entries",
            "count": int(len(alex)),
            "exclusion_reason": "full alex-mp v20 train+val rows available locally",
        },
        {
            "step": "MatterGen alex-mp-20 entries with MP identifiers",
            "count": int(alex["is_mp_identifier"].sum()),
            "exclusion_reason": "non-MP alex-mp-20 IDs cannot be queried through MP material_id join",
        },
        {
            "step": "Frozen Route B MP-ID candidates attempted",
            "count": int(len(matches_all)),
            "exclusion_reason": "predeclared minimal MP-vs-alex-mp-20 rescue subset, not full alex-mp-20 population",
        },
        {
...
def representativeness_tables(alex: pd.DataFrame, strict: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    for source, col in [("MP", "mp_e_above_hull"), ("alex-mp-20", "alex_e_above_hull")]:
    ...
    stable_fraction = pd.DataFrame(
        [
            {
                "population": "MatterGen_alex_mp_20_all_rows",
                "n": int(len(alex)),
                "stable_fraction_source_native": float(alex["alex_stable_exact"].mean()),
                "label_source": "alex-mp-20",
            },
            {
                "population": "MatterGen_alex_mp_20_MP_identifier_rows",
            ...
            {
                "population": "MP_alex_mp_20_strict_denominator_alex_mp_20_labels",
                "n": int(len(strict)),
