...
def test_full_mp_alex_mp20_43984_denominator_is_completed_and_scoped() -> None:
...
def test_denominator_construction_audit_explains_287_denominator() -> None:
    ...
    assert int(counts["MatterGen alex-mp-20 entries"]) == 675204
    assert int(counts["MatterGen alex-mp-20 entries with MP identifiers"]) == 43984
...
def test_representativeness_tables_are_present_and_scoped() -> None:
    ...
    assert "MatterGen_alex_mp_20_all_rows" in pops
    assert "MatterGen_alex_mp_20_MP_identifier_rows" in pops
    assert "MP_Alex_strict_denominator_MP_labels" in pops
    assert "MP_alex_mp_20_strict_denominator_alex_mp_20_labels" in pops
    ...
    assert {"MP", "alex-mp-20"}.issubset(set(ehull["source"]))
